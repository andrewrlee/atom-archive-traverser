...
/**
 * A Feed Traverser performs a callback method on each visited entry in order of
 * publishing.<br/>
 * <p>
 * It will traverse through each feed and follow prev-archive and next-archive
 * links to visit the entire history of events.
 * </p>
 * <p>
 * This walks backwards in time, through the feed until it finds an entry that
 * matches its starting predicate:
 * {@link FeedTraverserBuilder#foundStartingEntryWhen(Predicate)}, or if it
 * finishes traversing through all of the feeds. It then goes forwards in time,
 * processing entries that match
 * {@link FeedTraverserBuilder#processEntriesWhich(Predicate)}. will determine
 * which entries should be processed.
 * </p>
 * <br/>
 * <p>
 ...
 * <br/>
 * <p>
 * By default, all entries will be processed with a No-Operation EntryVisitor.
 * No particular entry will start forward traversal, it will travel backwards
 * until it reaches the first ever entry and then process each entry in turn
 * until the last published entry.
 * </p>
 */
public class FeedTraverserBuilder {
    ...
    /**
     * Create a feed traverser, using jersey client to retrieve the feeds
     */
    ...
    /**
     * Register a predicate that triggers forward processing of the feed. This
     * would generally match the last previously read entry that has been
     * processed. This defaults to {@link Predicates#alwaysFalse()} meaning the
     * traverser will visit all events.
     * 
    ...
    /***
     * Specify the order in which entries in the feed are processed - latest to
     * earliest. This defaults to {@link EntryFunctions#earliestToLatest()}
     * ensuring that all entries are ordered by updated date
     */
    ...
    /**
     * Process entries in the order in which they appear in the feed
     * 
     * @see {@link EntryFunctions#naturalOrdering()}
     */
    ...
    /**
     * Register an object to process entries. Defaults to {@Link
     * EntryVisitor#noopVisitor} which only logs each procesed entry.
     */
    public FeedTraverserBuilder whenFound(EntryVisitor visitor) {
